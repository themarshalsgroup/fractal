...
class EmberJsonApiSerializer extends ArraySerializer
{
    protected $baseUrl;
     ...
     */
    public function __construct($baseUrl = null, $requestedIncludes = null)
    {
    ...
    {
		if (empty($this->requestedIncludes)) { return []; }
		
		foreach ($this->requestedIncludes as $include) {
			$explodedIncludes = explode(".", $include);
			if (is_array($explodedIncludes)) {
				foreach ($explodedIncludes as $explodedInclude) {
					$requestedIncludes[] = $explodedInclude;
				}
			} else {
    ...
    private function fillRelationshipAsCollection($data, $relationship, $key)
    {
        foreach ($relationship as $index => $relationshipData) {
			...
			if (empty($relationshipData['data'])) { continue; }
